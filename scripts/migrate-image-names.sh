...
image_directory="${1:-/homeassistant/www/house-power-flow-card/images}"
dry_run="${2:-}"

if [[ ! -d "$image_directory" ]]; then
  echo "Image directory does not exist: $image_directory" >&2
  exit 1
fi
...
for entry in "${mapping[@]}"; do
  ...
  source_path="$image_directory/$source_name"
  destination_path="$image_directory/$destination_name"

  if [[ ! -f "$source_path" ]]; then
    if [[ ! -f "$destination_path" ]]; then
      echo "Warning: missing source (skipped): $source_name" >&2
    fi
    continue
  fi

  if [[ -e "$destination_path" ]]; then
    echo "Destination already exists; nothing was moved: $destination_path" >&2
    exit 1
  fi
  moves+=("$entry")
done
...
for entry in "${moves[@]}"; do
  ...
  mkdir -p -- "$(dirname -- "$image_directory/$destination_name")"
  mv -- "$image_directory/$source_name" "$image_directory/$destination_name"
done

printf 'Validated %d mappings; %s %d files.\n' "${#mapping[@]}" \
  "$([[ "$dry_run" == "--dry-run" ]] && echo 'would move' || echo 'moved')" \
  "${#moves[@]}"
echo 'No existing destination was overwritten.'
