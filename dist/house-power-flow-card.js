...
(() => {
  ...
  const TRANSLATIONS = {
    en: {
      ...
      solar: "Solar", grid: "Grid", from_grid: "From grid", to_grid: "To grid",
      gas: "Gas use", today: "Today", home: "Home use", current: "Current use",
      total_today: "Total today", battery: "Battery", standby: "Stand-by",
      active_yield: "Producing", no_yield: "No production", full: "Full", empty: "Empty",
      full_in: "Full in", empty_in: "Empty in", last_update: "Last update",
      self_sufficiency: "Self-sufficient", own_solar: "Self-consumed solar",
      solar_production: "Solar production", grid_import: "Grid import",
      grid_export: "Grid export", low_carbon: "Low-carbon electricity",
      support: "Support this project", close: "Close", donate: "Open support page"
    },
    nl: {
      title: "Energieoverzicht", title_test: "Energieoverzicht test", live_status: "Live status",
      solar: "PV-energie", grid: "Net", from_grid: "Van het net", to_grid: "Naar het net",
      gas: "Gasverbruik", today: "Vandaag", home: "Verbruik thuis", current: "Huidig verbruik",
      total_today: "Totaal vandaag", battery: "Batterij", standby: "Stand-by",
      active_yield: "Actieve opbrengst", no_yield: "Geen opbrengst", full: "Vol", empty: "Leeg",
      full_in: "Vol over", empty_in: "Leeg over", last_update: "Laatste update",
      self_sufficiency: "Zelfvoorzienend", own_solar: "Eigen zonverbruik",
      solar_production: "Zonproductie", grid_import: "Netafname",
      grid_export: "Teruglevering", low_carbon: "CO₂-arme elektriciteit",
      support: "Steun dit project", close: "Sluiten", donate: "Open donatiepagina"
    },
    de: { title: "Energieübersicht", title_test: "Energieübersicht Test", live_status: "Live-Status", solar: "Solar", grid: "Netz", battery: "Batterie", home: "Hausverbrauch", today: "Heute", support: "Projekt unterstützen" },
    fr: { title: "Vue d’ensemble énergétique", title_test: "Test de l’aperçu énergétique", live_status: "État en direct", solar: "Solaire", grid: "Réseau", battery: "Batterie", home: "Consommation maison", today: "Aujourd’hui", support: "Soutenir le projet" },
    es: { title: "Resumen de energía", title_test: "Prueba del resumen de energía", live_status: "Estado en directo", solar: "Solar", grid: "Red", battery: "Batería", home: "Consumo del hogar", today: "Hoy", support: "Apoyar el proyecto" }
  };
  ...
  class HousePowerFlowCard extends HTMLElement {
    constructor() {
      super();
      ...
      this._onSupportClick = (event) => {
        event.preventDefault();
        event.stopPropagation();
        this._supportDialog?.classList.add("open");
      };
      this._onSupportClose = (event) => {
        event?.preventDefault();
        event?.stopPropagation();
        this._supportDialog?.classList.remove("open");
      };
    }
    ...
    setConfig(config) {
      ...
      this._config = {
        ...
        language: "auto",
        background_image: "",
        image_directory: "/hacsfiles/House-Powerflow-Card/images",
        image_version: "",
        ...
        support: {
          ...
          message: "Like the card? A small donation helps future development."
        },
        ...config,
      ...
      const supportTitle = this.shadowRoot?.querySelector("[data-support-title]");
      const supportMessage = this.shadowRoot?.querySelector("[data-support-message]");
      const supportLink = this.shadowRoot?.querySelector("[data-support-link]");
      if (supportTitle) supportTitle.textContent = this._config.support.title || this._t("support");
      if (supportMessage) supportMessage.textContent = this._config.support.message || "";
      if (supportLink) supportLink.href = this._config.support.url;
    ...
    set hass(hass) {
      ...
      if (previousLanguage !== this._language()) this._applyTranslations();
    ...
    static getConfigElement() {
      return document.createElement("house-power-flow-card-editor");
    }

    _language() {
      const requested = String(this._config?.language || "auto").toLowerCase();
      const language = requested === "auto"
        ? String(this._hass?.language || this._hass?.locale?.language || "en")
        : requested;
      return TRANSLATIONS[language.split("-")[0]] ? language.split("-")[0] : "en";
    }
    ...
    _displayTitle() {
      ...
      if (!title) return this._t("title");
      if (this._config?.translate_title === false) return title;
      const normalized = title.toLocaleLowerCase().replaceAll(/\s+/g, " ");
      if (["energieoverzicht test", "energy overview test"].includes(normalized)) return this._t("title_test");
      if (["energieoverzicht", "energy overview", "house power flow"].includes(normalized)) return this._t("title");
      return title;
    }

    _displaySubtitle() {
      const subtitle = String(this._config?.subtitle || "").trim();
      if (!subtitle || ["live status", "live-status", "état en direct", "estado en directo"].includes(subtitle.toLocaleLowerCase())) return this._t("live_status");
      return subtitle;
    }

    _applyTranslations() {
      if (!this._shellBuilt) return;
      this._setText("title", this._displayTitle());
      this._setText("subtitle", this._displaySubtitle());
      this.shadowRoot.querySelectorAll("[data-i18n]").forEach((element) => {
        element.textContent = this._t(element.dataset.i18n);
      });
      this._supportButton?.setAttribute("title", this._t("support"));
      this._supportButton?.setAttribute("aria-label", this._t("support"));
    }

    _moduleEnabled(name) {
      const setting = this._config?.modules?.[name];
      if (setting === true || setting === false) return setting;
      if (name === "battery") return Boolean(this._config?.battery_charge_power || this._config?.battery_discharge_power || this._config?.battery_soc);
      if (name === "gas") return Boolean(this._config?.gas_today);
      if (name === "low_carbon") return Boolean(this._config?.low_carbon_entity);
      return true;
    }

    _applyModuleVisibility() {
      if (!this._shellBuilt) return;
      for (const name of ["solar", "grid", "battery", "gas", "low_carbon"]) {
    ...
    _buildShell() {
      ...
      this.shadowRoot.innerHTML = `
        ...
        <ha-card id="card" tabindex="0" role="link">
          <div class="canvas">
            ...
            <section class="panel solar-panel" data-module="solar">
              <div class="panel-icon solar-icon">
                <ha-icon icon="mdi:solar-power"></ha-icon>
              </div>
              <div>
                <div class="panel-label" data-i18n="solar">PV-energie</div>
                <div class="panel-value solar-value" data-value="solar-power">--</div>
                <div class="panel-sub" data-value="solar-status">--</div>
              </div>
            </section>

            <section class="panel grid-panel" data-module="grid">
              <div class="panel-icon grid-icon">
                <ha-icon icon="mdi:transmission-tower"></ha-icon>
              </div>
              <div>
                <div class="panel-label" data-i18n="grid">Net</div>
                <div class="grid-import" data-value="grid-import">← --</div>
                <div class="grid-caption" data-i18n="from_grid">Van het net</div>
                <div class="grid-export" data-value="grid-export">→ --</div>
                <div class="grid-caption" data-i18n="to_grid">Naar het net</div>
              </div>
            </section>

            <section class="panel gas-panel" data-module="gas">
              <div class="panel-icon gas-icon">
                <ha-icon icon="mdi:fire"></ha-icon>
              </div>
              <div>
                <div class="panel-label" data-i18n="gas">Gasverbruik</div>
                <div class="panel-value gas-value" data-value="gas-today">--</div>
                <div class="panel-sub" data-i18n="today">Vandaag</div>
              </div>
            </section>

            <section class="panel home-panel">
              <button id="support-button" class="support-button" type="button" aria-label="Support">
              ...
              <div class="home-label" data-i18n="home">Verbruik thuis</div>
              <div class="home-value" data-value="home-power">--</div>
              <div class="home-sub" data-i18n="current">Huidig verbruik</div>
              <div class="home-total">
                <span data-i18n="total_today">Totaal vandaag</span>
                <strong data-value="home-total">--</strong>
              </div>
            </section>

            <section class="panel battery-panel" data-module="battery">
              <div class="battery-icon">
                <ha-icon icon="mdi:battery-high"></ha-icon>
              </div>
              <div class="battery-data">
                <div class="battery-title">
                  <span data-i18n="battery">Batterij</span>
            ...
            <div class="updated">
              <span data-i18n="last_update">Laatste update</span>: <span data-value="updated">--:--</span>
            </div>

            <div id="support-dialog" class="support-dialog" role="dialog" aria-modal="true">
              <div class="support-dialog-card" role="document">
                <button id="support-close" class="support-close" type="button" aria-label="Close">×</button>
                <ha-icon icon="mdi:heart" class="support-heart"></ha-icon>
                <h2 data-support-title></h2>
                <p data-support-message></p>
                <a data-support-link target="_blank" rel="noopener noreferrer" data-i18n="donate">Open support page</a>
              </div>
            </div>
      ...
      this._supportButton?.addEventListener("click", this._onSupportClick);
      this._supportClose?.addEventListener("click", this._onSupportClose);
      this._supportDialog?.addEventListener("click", (event) => {
        if (event.target === this._supportDialog) this._onSupportClose(event);
      });
    ...
    _kpiMarkup(key, icon, color, label, translationKey, module = "") {
      return `
        <div class="kpi" ${module ? `data-module="${module}"` : ""}>
          <div class="kpi-icon ${color}">
            <ha-icon icon="${icon}"></ha-icon>
          </div>
          <div class="kpi-text">
            <div class="kpi-label" data-i18n="${translationKey}">${label}</div>
    ...
    _cacheElements() {
      ...
      this._supportButton = this.shadowRoot.getElementById("support-button");
      this._supportDialog = this.shadowRoot.getElementById("support-dialog");
      this._supportClose = this.shadowRoot.getElementById("support-close");
      const support = this._config?.support || {};
      const title = this.shadowRoot.querySelector("[data-support-title]");
      const message = this.shadowRoot.querySelector("[data-support-message]");
      const link = this.shadowRoot.querySelector("[data-support-link]");
      if (title) title.textContent = support.title || this._t("support");
      if (message) message.textContent = support.message || "";
      if (link) link.href = support.url || "https://ko-fi.com/timverweij92";
    ...
    _cyclesPerSecond(power) {
      ...
      const speedMultiplier = this._animationSpeed();
    ...
    _animationSpeed() {
      const configured = Number(this._config?.animation_speed);
      return Number.isFinite(configured)
        ? Math.min(10, Math.max(0.1, configured))
        : 0.5;
    }
    ...
    _updateLiveValues() {
      ...
      let batteryStatus = this._t("standby");
      let batteryPower = 0;
      ...
      if (charging > 10) {
        ...
        batteryStatus = remainingKwh <= 0.01
          ? this._t("full")
          : `${this._t("full_in")} ${this._formatRemainingTime(eta)}`;
        ...
        batteryStatus = availableKwh <= 0.01
          ? this._t("empty")
          : `${this._t("empty_in")} ${this._formatRemainingTime(eta)}`;
      }

      this._setText("solar-power", this._formatPower(solar));
      this._setText(
        "solar-status",
        solar > 20 ? this._t("active_yield") : this._t("no_yield")
      );
      ...
      this._setText(
        "updated",
        new Date().toLocaleTimeString(this._language(), {
          hour: "2-digit",
          minute: "2-digit",
        })
      );
    ...
    _setFlowTarget(flow, power, active, direction) {
      ...
      const sharedSpeed = this._animationSpeed();
      const duration = Math.max(1.2, Math.min(120, 12 / sharedSpeed));
    ...
    _holidayDateFormat() {
      ...
      if (["european", "eu", "dd-mm"].includes(configured)) return "european";
      const locale = String(
      ...
      return /^en-US\b/i.test(locale) ? "american" : "european";
    }

    _normalizeHolidayDate(value, format = this._holidayDateFormat()) {
      const match = /^(\d{1,2})-(\d{1,2})$/.exec(String(value || "").trim());
      if (!match) return "";
      const first = Number(match[1]);
      const second = Number(match[2]);
      const month = format === "american" ? first : second;
      const day = format === "american" ? second : first;
      if (month < 1 || month > 12 || day < 1 || day > 31) return "";
      return `${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
    }

    _dateInRange(now, startValue, endValue, format = this._holidayDateFormat()) {
      const start = this._normalizeHolidayDate(startValue, format);
      const end = this._normalizeHolidayDate(endValue, format);
      if (!start || !end) return false;
      const value = `${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
      return start <= end ? value >= start && value <= end : value >= start || value <= end;
    }

    _holidayItemEnabled(name) {
      return this._config?.holidays?.items?.[name]?.enabled !== false;
    }
    ...
    _coordinatesToPath(points) {
      ...
      return valid.map((point, index) => `${index ? "L" : "M"} ${Number(point[0])} ${Number(point[1])}`).join(" ");
    }

    _applyCustomLayout() {
      const coordinates = this._config?.layout?.coordinates || {};
      const paths = this._config?.layout?.paths || this._config?.flow_paths || {};
      let applied = false;
      for (const [flow, element] of Object.entries(this._paths)) {
        const path = typeof paths[flow] === "string" ? paths[flow] : this._coordinatesToPath(coordinates[flow]);
        if (path && element) {
    ...
    _styles() {
      return `
        ...
        /* Houd het huisbeeld rustig wanneer zon of batterij niets levert.
           De complete route (basislijn, gloed en puls) komt pas terug zodra
           de bijbehorende energiestroom werkelijk actief is. */
        .flow-visual.solar,
        .flow-visual.battery {
          opacity: 0;
        ...
        .flow-visual.solar.active,
        .flow-visual.battery.active {
          opacity: 1;
        }
        ...
        .support-button ha-icon { --mdc-icon-size: 1.35cqw; }
        .support-dialog {
          position: absolute;
          z-index: 30;
          inset: 0;
        ...
        .support-dialog-card h2 { margin: 8px 0; font-size: clamp(18px, 1.8cqw, 28px); }
        .support-dialog-card p { margin: 0 0 18px; color: rgba(235,241,248,.8); }
        .support-dialog-card a {
          display: inline-block;
          padding: 10px 16px;
          border-radius: 999px;
          color: #fff;
        ...
        .support-heart { color: #ff6f96; --mdc-icon-size: 38px; }
        .support-close {
          position: absolute;
          top: 8px;
          right: 10px;
  ...
  class HousePowerFlowCardEditor extends HTMLElement {
    set hass(hass) {
      this._hass = hass;
    ...
    _render() {
      ...
      this._activeTab = this._activeTab || "basic";
      if (!this.shadowRoot) this.attachShadow({ mode: "open" });
      this.shadowRoot.innerHTML = `
        <style>
          .editor { display: grid; gap: 14px; padding: 8px 0; }
          .tabs { display: grid; grid-template-columns: repeat(3, 1fr); gap: 4px; position: sticky; top: 0; z-index: 5; padding: 0 0 8px; background: var(--card-background-color); }
          .tab { padding: 10px 6px; border: 0; border-bottom: 2px solid transparent; color: var(--secondary-text-color); background: transparent; cursor: pointer; font-weight: 600; }
          .tab.active { color: var(--primary-color); border-bottom-color: var(--primary-color); }
          .tab-page { display: none; gap: 14px; }
          .tab-page.active { display: grid; }
          h3 { margin: 12px 0 0; font-size: 16px; }
          label { display: grid; gap: 6px; color: var(--primary-text-color); }
          input, select { box-sizing: border-box; width: 100%; padding: 11px; border: 1px solid var(--divider-color); border-radius: 8px; color: var(--primary-text-color); background: var(--card-background-color); }
          .toggle { display: flex; align-items: center; justify-content: space-between; }
          small { color: var(--secondary-text-color); }
          .holiday { display: grid; gap: 10px; padding: 12px; border: 1px solid var(--divider-color); border-radius: 10px; }
          .holiday-dates { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        </style>
        <div class="editor">
          <div class="tabs">
            ${[["basic", "Basic"], ["meters", "Extra meters"], ["holidays", "Holidays"]].map(([id, label]) => `<button type="button" class="tab ${this._activeTab === id ? "active" : ""}" data-tab="${id}">${label}</button>`).join("")}
          </div>

          <div class="tab-page ${this._activeTab === "basic" ? "active" : ""}" data-page="basic">
            <label>Title<input data-field="title" placeholder="Automatically translated" value="${this._escape(this._config.title || "")}"></label>
            <small>Leave the title empty for an automatically translated title.</small>
            <label>Language<select data-field="language">
              ${["auto", "en", "nl", "de", "fr", "es"].map((language) => `<option value="${language}" ${String(this._config.language || "auto") === language ? "selected" : ""}>${language}</option>`).join("")}
            </select></label>
            <label>Flow animation speed (0.1–10.0)
              <input type="number" data-field="animation_speed" min="0.1" max="10" step="0.1" value="${this._escape(this._config.animation_speed ?? 0.5)}">
            </label>
          ...
          <div class="tab-page ${this._activeTab === "meters" ? "active" : ""}" data-page="meters">
            <h3>Daily totals</h3>
            ${this._entityPicker("solar_production_today", "Solar production today")}
            ${this._entityPicker("grid_import_today", "Grid import today")}
            ${this._entityPicker("grid_export_today", "Grid export today")}
            <h3>Battery</h3>
            <label>Battery visibility<select data-module-setting="battery">
              ${this._visibilityOptions("battery")}
            </select></label>
            ${this._entityPicker("battery_charge_power", "Battery charge power")}
            ${this._entityPicker("battery_discharge_power", "Battery discharge power")}
            ${this._entityPicker("battery_soc", "Battery state of charge")}
            <h3>Gas</h3>
            <label>Gas visibility<select data-module-setting="gas">
              ${this._visibilityOptions("gas")}
            </select></label>
            ${this._entityPicker("gas_today", "Gas consumption today")}
          </div>
      ...
      this.shadowRoot.querySelectorAll("input[data-field], select[data-field]").forEach((element) => element.addEventListener("change", (event) => this._updateField(event.currentTarget.dataset.field, event.currentTarget.value)));
      this.shadowRoot.querySelectorAll("ha-entity-picker[data-field]").forEach((element) => element.addEventListener("value-changed", (event) => this._updateField(event.currentTarget.dataset.field, event.detail?.value || "")));
      this.shadowRoot.querySelectorAll("[data-module-setting]").forEach((element) => element.addEventListener("change", (event) => {
        const value = event.currentTarget.value === "auto" ? "auto" : event.currentTarget.value === "true";
        this._updateNested(`modules.${event.currentTarget.dataset.moduleSetting}`, value);
      }));
      this.shadowRoot.querySelectorAll("[data-nested]").forEach((element) => element.addEventListener("change", (event) => this._updateNested(event.currentTarget.dataset.nested, event.currentTarget.checked)));
      this.shadowRoot.querySelectorAll("[data-nested-value]").forEach((element) => element.addEventListener("change", (event) => this._updateNested(event.currentTarget.dataset.nestedValue, event.currentTarget.value)));
    ...
    _visibilityOptions(module) {
      return [["auto", "Automatic"], ["true", "Always show"], ["false", "Hidden"]]
        .map(([value, label]) => `<option value="${value}" ${String(this._config.modules?.[module] ?? "auto") === value ? "selected" : ""}>${label}</option>`)
        .join("");
    }

    _editorHolidayFormat() {
      const configured = String(this._config.holidays?.date_format || "auto");
      if (configured !== "auto") return configured;
      const locale = String(this._hass?.locale?.language || this._hass?.language || navigator.language || "en").replace("_", "-");
      return /^en-US\b/i.test(locale) ? "american" : "european";
    }

    _holidayDateValue(name, field) {
      const custom = this._config.holidays?.items?.[name]?.[field];
      if (custom) return custom;
      const mmdd = DEFAULT_HOLIDAYS[name]?.[field] || "";
      if (!mmdd || this._editorHolidayFormat() === "american") return mmdd;
    ...
    _holidayEditorMarkup() {
      ...
      const movable = [
        ["pasen", "Easter"], ["moederdag", "Mother's Day"],
        ["hemelvaart", "Ascension Day"], ["pinksteren", "Pentecost"],
        ["vaderdag", "Father's Day"]
      ];
      const fixedMarkup = fixed.map(([name, label]) => `
        <div class="holiday">
          <label class="toggle">${label}<input type="checkbox" data-holiday-enable="${name}" ${this._config.holidays?.items?.[name]?.enabled !== false ? "checked" : ""}></label>
          <div class="holiday-dates">
            <label>Start<input data-holiday-date="${name}" data-date-field="start" value="${this._escape(this._holidayDateValue(name, "start"))}"></label>
            <label>End<input data-holiday-date="${name}" data-date-field="end" value="${this._escape(this._holidayDateValue(name, "end"))}"></label>
          </div>
        </div>`).join("");
      const movableMarkup = movable.map(([name, label]) => `
        <div class="holiday">
    ...
    _entityPicker(field, label, domain = "sensor") {
      return `<label>${label}<ha-entity-picker data-field="${field}" data-domain="${domain}" value="${this._escape(this._config[field] || "")}"></ha-entity-picker></label>`;
    }
    ...
    _updateField(field, value) {
      const normalizedValue = field === "animation_speed"
        ? Math.min(10, Math.max(0.1, Number(value) || 0.5))
        : value;
      const batteryFields = ["battery_charge_power", "battery_discharge_power", "battery_soc"];
      const shouldEnableBattery = batteryFields.includes(field) && Boolean(normalizedValue);
      const shouldEnableGas = field === "gas_today" && Boolean(normalizedValue);
      this._config = {
        ...this._config,
        [field]: normalizedValue,
        ...((shouldEnableBattery || shouldEnableGas) ? {
          modules: {
            ...(this._config.modules || {}),
            ...(shouldEnableBattery ? { battery: "auto" } : {}),
            ...(shouldEnableGas ? { gas: "auto" } : {}),
          }
        } : {}),
      };
      this._changed();
    ...
    _updateNested(path, value) {
      ...
      this._config = { ...this._config, [group]: { ...(this._config[group] || {}), [field]: value } };
      this._changed();
    }

    _updateHoliday(name, field, value) {
  ...
  if (!customElements.get("house-power-flow-card-editor")) customElements.define("house-power-flow-card-editor", HousePowerFlowCardEditor);
  customElements.define(CARD_TAG, HousePowerFlowCard);
